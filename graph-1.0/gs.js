class GraphLang extends HTMLElement {
    ...
    parse(input) {
        ...
        const backgroundColorRegex = /\[back\](#[0-9A-Fa-f]{6})\[\/back\]/g;
        const backgroundImageRegex = /\[backimg\](https?:\/\/[^\s]+)\[\/backimg\]/g;
        ...
        const combinedStyleRegex = /\{(.*?)\}(.+?)\{\/\}/g;
        const sizeRegex = /\{(\w+)\+\(.*?size:\s*(\d+);.*?\)\}(.+?)\{\/\}/g;

        input = input.replace(sizeRegex, (match, styles, size, text) => {
            let sizeStyle = '';
            if (size) {
                sizeStyle = `font-size: ${size}em; `;
            }
            return `<span style="${sizeStyle}">${text}</span>`;
        });

        input = input.replace(backgroundColorRegex, (match, color) => {
            return `<div style="background-color: ${color}; padding: 10px;">`;
        });

        input = input.replace(combinedStyleRegex, (match, styles, text) => {
            const styleArray = styles.split(',').map(style => style.trim());
            let styleString = '';

            styleArray.forEach(style => {
                if (style.startsWith('color:')) {
                    styleString += `color: ${style.split(':')[1].trim()}; `;
                } else if (style.startsWith('font:')) {
                    styleString += `font-family: ${style.split(':')[1].trim()}; `;
                } else if (style === 'shadow') {
                    styleString += 'text-shadow: 1px 1px 2px rgba(0,0,0,0.5); ';
                } else if (style === 'b') {
        ...
        input = input.replace(headingRegex, (match, level, text) => `<h${level}>${text}</h${level}>`);
        input = input.replace(titleRegex, (match, title) => `<title>${title}</title>`);
        input = input.replace(linkRegex, (match, text, url) => `<a href="${url}">${text}</a>`);
